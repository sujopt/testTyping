...
function getRandomFromPool(pool: readonly string[]): string {
...
export async function fetchSentence(difficulty: DifficultyMode): Promise<string> {
  ...
  return getRandomFromPool(SENTENCES_BY_DIFFICULTY[resolvedDifficulty])
}

export async function fetchParagraph(difficulty: DifficultyMode): Promise<string> {
  const resolvedDifficulty = normalizeDifficulty(difficulty)
  return getRandomFromPool(PARAGRAPHS_BY_DIFFICULTY[resolvedDifficulty])
}
